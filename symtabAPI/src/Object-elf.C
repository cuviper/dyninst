                           ...
                           std::vector<ExceptionBlock> &addresses)
{
    ...
    uint8_t *fde_augdata, *cie_augdata;
    Dwarf_Word fde_augdata_len, cie_augdata_len;
    ...
    for (int i = 0; i < fde_count; i++) {
        ...
        //Get the FDE
        status = dwarf_get_fde_n(fde_data, (Dwarf_Word) i, &fde, &err);
        if (status != DW_DLV_OK) {
            pd_dwarf_handler();
            return false;
        }
