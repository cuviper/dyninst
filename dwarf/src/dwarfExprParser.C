...
bool decodeDwarfExpression(Dwarf_Op *expr, size_t exprlen,
                           long int *initialStackValue,
                           VariableLocation &loc,
                           Dyninst::Architecture arch) {
   SymbolicDwarfResult res(loc, arch);
   if (!decodeDwarfExpression(expr, exprlen, initialStackValue, 
      ...
      switch( locations[i].atom ) {
         ...
         case DW_OP_skip: {
            ...
            if ( bytes < 0 ) {
               ...
               for ( j = i + 1; (unsigned)j < exprlen; j ++ ) {
                  if ( locations[j].offset == target ) { break; }
               } /* end search forward */
